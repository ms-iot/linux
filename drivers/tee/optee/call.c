...
static int verify_ocall_request(u32 num_params, struct optee_call_ctx *call_ctx)
{
	...
	case OPTEE_MSG_RPC_CMD_SHM_FREE:
		...
		/*
		 * Ensure that we won't read past the end of the SHM underlying
		 * arg->params.
		 */
		sz = sizeof(*arg) + sizeof(*arg->params) * arg->num_params;
		if (sz > call_ctx->rpc_shm->size)
			return -EINVAL;
			       ...
			       struct optee_call_ctx *call_ctx)
{
	...
	case TEE_IOCTL_OCALL_CMD_SHM_ALLOC:
		...
		/* Could be NULL and zero, respectively */
		call_ctx->ocall_pages_list = shm_pages_list;
		call_ctx->ocall_num_entries = shm->num_pages;
		break;
	case TEE_IOCTL_OCALL_CMD_SHM_FREE:
		...
		if (call_ctx->ocall_pages_list) {
			optee_free_pages_list(call_ctx->ocall_pages_list,
					      call_ctx->ocall_num_entries);
			call_ctx->ocall_pages_list = NULL;
			call_ctx->ocall_num_entries = 0;
		}
		break;
	case TEE_IOCTL_OCALL_CMD_INVOKE:
		/* -2 here and +2 below to skip the OCALL descriptors */
