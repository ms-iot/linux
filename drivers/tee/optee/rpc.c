...
static struct tee_shm *cmd_alloc_host(struct tee_context *ctx, size_t sz, struct optee_session *session)
{
	...
	ret = optee_grpc_thrd_req(session, OPTEE_MSG_RPC_CMD_SHM_ALLOC, 1, &param);
	if (ret)
		return ERR_PTR(-ENOMEM);
...
static void cmd_free_host(struct tee_shm *shm, struct optee_session *session)
{
	...
	optee_grpc_thrd_req(session, OPTEE_MSG_RPC_CMD_SHM_FREE, 1, &param);
}
					...
					struct optee_call_ctx *call_ctx)
{
	...
	arg->ret = optee_grpc_thrd_req(sess, arg->cmd, arg->num_params, params);
